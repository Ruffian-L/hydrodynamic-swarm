...
impl Lens {
    ...
    pub fn transport(&self, residual: &Tensor, layer: usize, band: &BandId) -> Result<Tensor> {
        let block = self.blocks.get(&(layer, band.clone())).ok_or_else(|| {
            anyhow::anyhow!("lens has no transport for layer {layer} band {band}")
        })?;
    ...
    pub fn apply(
        ...
        let select = |captured: Option<Tensor>, what: &str| -> Result<Tensor> {
            let h = captured.ok_or_else(|| anyhow::anyhow!("no activation captured for {what}"))?;
            Ok(h.narrow(0, 0, 1)?
                .index_select(&idx, 1)?
                .squeeze(0)?
                .to_dtype(DType::F32)?)
    ...
    pub fn save(&self, path: &Path) -> Result<()> {
        ...
        for ((layer, band), block) in &self.blocks {
            ...
            if let Some(u) = &block.u {
                tensors.push((
                    format!("U/{layer}/{band}"),
                    TensorView::new(Dtype::F32, vec![block.rank, self.d_model], bytemuck_cast(u))?,
                ));
            }
        }
