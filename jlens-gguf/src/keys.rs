...
use hydrodynamic_swarm::jacobian::{text_bridge_hash, DimSignature, JacobianKey, KeyPhase};
...
mod tests {
    ...
    fn only_phase_named_bands_map_to_a_phase() {
        assert_eq!(
            phase_for_band(&BandId::named("revise")),
            Some(KeyPhase::Revise)
        );
        assert_eq!(
            phase_for_band(&BandId::named("settle")),
            Some(KeyPhase::Settle)
        );
        assert_eq!(
            phase_for_band(&BandId::named("answer")),
            Some(KeyPhase::Answer)
        );
    ...
    fn top_dims_rank_by_magnitude_not_sign() {
        let transported = Tensor::from_vec(vec![0.1f32, -5.0, 2.0, 0.0], 4, &Device::Cpu).unwrap();
