...
use hydrodynamic_swarm::loader::Model;
...
pub fn valid_positions(seq_len: usize, skip_first: usize) -> Result<Vec<usize>> {
    if seq_len <= skip_first + 1 {
        bail!(
            "prompt too short: seq_len={seq_len}, need > {}",
            skip_first + 1
        );
    }
    ...
    pub fn parse(spec: &str) -> Result<Self> {
        match spec {
            ...
            other => {
                ...
                let labels: Vec<Vec<String>> = serde_json::from_str(&raw).with_context(|| {
                    format!("parsing {path} as [[String]] (one array per prompt)")
                })?;
                if labels.is_empty() {
        ...
        let bands = match self {
            ...
            Self::Thirds => {
                ...
                ["early", "middle", "late"]
                    .iter()
                    .enumerate()
                    .map(|(i, name)| (BandId::named(name), valid[cut[i]..cut[i + 1]].to_vec()))
                    .collect()
            }
            Self::Labels(labels) => {
...
fn prefill(
    model: &mut Model,
    tokens: &Tensor,
    hook: &mut dyn hydrodynamic_swarm::hooks::LayerHook,
) -> Result<()> {
...
pub fn build_bases(
    ...
    let sites: Vec<Site> = cfg
        .source_layers
        .iter()
        .map(|&l| Site::block_out(l))
        .collect();

    for (i, prompt) in prompts.iter().enumerate() {
        if samples
            .values()
            .all(|s| s.len() >= target_samples * d_model)
        {
            break;
        }
...
pub fn fit(
    ...
    let capture_sites: Vec<Site> = cfg
        .source_layers
        .iter()
        .map(|&l| Site::block_out(l))
        .collect();
...
mod tests {
    ...
    fn valid_positions_match_the_reference_mask() {
        ...
        assert!(
            valid_positions(17, 16).is_err(),
            "seq_len 17 leaves no valid positions"
        );
    }
    ...
    fn thirds_partition_without_overlap_or_loss() {
        ...
        assert_eq!(
            all, valid,
            "thirds must cover every valid position exactly once"
        );
    }
    ...
    fn labels_group_by_name_and_reject_short_rows() {
        ...
        assert!(
            short.bands(&valid, 0).is_err(),
            "labels must cover every token"
        );
    }
}
