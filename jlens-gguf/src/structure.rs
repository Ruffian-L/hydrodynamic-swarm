...
mod tests {
    ...
    fn silhouette_is_high_for_real_blobs_and_low_after_shuffling() {
        ...
        assert!(
            real > 0.8,
            "well-separated blobs should score high, got {real}"
        );
        assert!(real > shuffled, "real {real} must beat shuffled {shuffled}");
    }
    ...
    fn participation_ratio_drops_when_structure_is_present() {
        ...
        assert!(
            real < 2.5,
            "structured data should concentrate variance, got {real}"
        );
    }
