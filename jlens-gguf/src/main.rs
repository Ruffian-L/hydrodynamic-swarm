...
use hydrodynamic_swarm::loader::{self, Model};
...
fn source_rms(
    ...
    use hydrodynamic_swarm::hooks::LayerHook;
...
fn cmd_structure(
    ...
    if let (Some(path), Some(group_col)) = (tags_path, pair_by) {
        ...
        let gi =
            col(group_col).ok_or_else(|| anyhow::anyhow!("tags have no column {group_col:?}"))?;
        ...
        for &layer in &layers {
            ...
            let sc = jlens_gguf::stability::LayerScores {
                layer,
                positive,
                null,
            }
...
fn cmd_stability_decode(
    ...
    use hydrodynamic_swarm::jacobian::DimSignature;
    ...
    if !anchors.is_empty() {
        ...
        println!(
            "\nanchor step: mean {mean:.1} over {} prompts ({skipped} unanchored)",
            anchors.len()
        );
    }
    ...
    let deterministic = match &determinism {
        Some((a, b)) => {
            ...
            println!(
                "criterion 3 — determinism: greedy decode twice -> {}",
...
fn cmd_stability(
    ...
    let raw = std::fs::read_to_string(corpus_path)
        .with_context(|| format!("reading corpus {}", corpus_path.display()))?;
    let corpus: stability::Corpus = serde_json::from_str(&raw).with_context(|| {
        format!(
            "parsing {} as {{subject: [paraphrase]}}",
            corpus_path.display()
        )
    })?;
    if corpus.len() < 2 {
        bail!(
            "the null population needs at least 2 subjects, got {}",
            corpus.len()
        );
    }
    ...
    println!("commit position = last prompt token - {commit_offset} (pre-first-generated)\n");
        ...
        for (idx, prompt) in paraphrases.iter().enumerate() {
            ...
            for _ in 0..repeats {
                ...
                for &layer in &layers {
                    ...
                    let readout = keys::read_out(&ranked, &logits, &tokenizer, top_k, top_dims)?;

                    let signature = DimSignature::from_top_dimensions(&readout.top_dims, top_dims);
                    signatures
                        .entry(layer)
                        .or_default()
                        .entry(subject.clone())
            ...
            if repeats == 2 {
                ...
                for &layer in &layers {
                    if let Some(v) = signatures.get_mut(&layer).and_then(|m| m.get_mut(subject)) {
                        v.pop();
                    }
    ...
    let deterministic = match &determinism_probe {
        Some((a, b)) => {
            let score = hydrodynamic_swarm::jacobian::weighted_jaccard(a, b);
    ...
    for &layer in &layers {
        ...
        let as_sigs: BTreeMap<String, Vec<DimSignature>> = per_subject
            ...
            .map(|(subject, rows)| {
                let sigs = rows
                    .into_iter()
                    .map(|ids| DimSignature::new(ids.into_iter().map(|id| (id, 1.0)).collect()))
                    .collect();
                (subject, sigs)
            })
            .collect();
    ...
    for &layer in &layers {
        ...
        let scores = jlens_gguf::stability::LayerScores {
            layer,
            positive,
            null,
        };
...
fn cmd_readout(
    ...
    if verify {
        ...
        let words: Vec<String> = readout
            ...
            .collect();
        eprintln!(
            "\nverify: model.forward() top-{top_k} = {}",
            words.join(" ")
        );
        eprintln!(
...
fn cmd_apply(
    ...
    let positions: Vec<i64> = positions_spec
        ...
        .map(|s| {
            s.trim()
                .parse::<i64>()
                .context("position must be an integer")
        })
        .collect::<Result<Vec<_>>>()?;
