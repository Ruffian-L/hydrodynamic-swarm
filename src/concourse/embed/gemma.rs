...
impl GemmaModel {
    ...
    pub fn load(config: GemmaConfig) -> SwarmResult<Self> {
        let device = match config.device.as_str() {
            "cuda" => {
                Device::new_cuda(0).map_err(|e| SwarmError::Embedding(format!("CUDA: {e}")))?
            }
            "metal" => {
                Device::new_metal(0).map_err(|e| SwarmError::Embedding(format!("Metal: {e}")))?
            }
            _ => Device::Cpu,
        };
...
impl EmbeddingGemma300M {
    pub fn new(model_path: &str) -> SwarmResult<Self> {
        ...
        Ok(Self {
            model: Arc::new(GemmaModel::load(config)?),
        })
    }
    pub fn embed(&self, text: &str) -> SwarmResult<Vec<f32>> {
...
mod tests {
    ...
    fn test_gemma_model_loading() {
        ...
        assert!(
            (norm - 1.0).abs() < 0.01,
            "Embedding should be L2-normalized (norm={norm:.4})"
        );
    ...
    fn test_cosine_similarity() {
        ...
        let c = model
            .embed("Quantum entanglement in superconductors")
            .unwrap();
