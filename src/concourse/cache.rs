...
impl LruCache {
    ...
    pub fn get(&mut self, key: &str) -> Option<&CacheEntry> {
        ...
        if let Some(pos) = pos {
            // ⚡ Bolt: reuse existing string to avoid allocation during LRU update
            let owned_key = self.access_order.remove(pos).unwrap();
            self.access_order.push_back(owned_key);
        } else {
            // Should not be reachable for a valid entry, but fallback safely
