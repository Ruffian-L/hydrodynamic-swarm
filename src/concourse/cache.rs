...
impl TtlCache {
    ...
    pub fn get(&mut self, key: &str) -> Option<&CacheEntry> {
        if let Some(entry) = self.entries.get(key) {
            // ⚡ Bolt: Check for expiration without allocating a String for the lookup key
            if entry.is_expired() {
...
impl CacheManager {
    ...
    pub fn get_embedding(&self, text: &str) -> Result<Option<Vec<f32>>> {
        ...
        if let Some(entry) = self
            .lru_cache
            .write()
            .map_err(|_| anyhow!("Failed to acquire write lock for LRU cache"))?
            .get(&cache_key)
        {
        ...
        if let Some(entry) = self
            .ttl_cache
            .write()
            .map_err(|_| anyhow!("Failed to acquire write lock for TTL cache"))?
            .get(&cache_key)
        {
    ...
    pub fn get_stats(&self) -> Result<HashMap<String, CacheStats>> {
        ...
        stats.insert(
            ...
            self.lru_cache
                .read()
                .map_err(|_| anyhow!("Failed to acquire read lock for LRU cache"))?
                .stats(),
        );
        stats.insert(
            "ttl_cache".to_string(),
            self.ttl_cache
                .read()
                .map_err(|_| anyhow!("Failed to acquire read lock for TTL cache"))?
                .stats(),
        );
        Ok(stats)
    }
