...
impl LruCache {
    ...
    pub fn get(&mut self, key: &str) -> Option<&CacheEntry> {
        ...
        // ⚡ Bolt: avoid allocating a new String on cache hits by reusing the removed key
        let pos = self.access_order.iter().position(|k| k == key);
        let key_str = if let Some(pos) = pos {
...
impl TtlCache {
    ...
    /// Get value from cache
    pub fn get(&mut self, key: &str) -> Option<&CacheEntry> {
        // ⚡ Bolt: avoid allocating a new String on cache hits by using get() instead of entry()
