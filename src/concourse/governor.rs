...
impl ActiveCell {
    ...
    pub fn add_edge(&mut self, tuple: FluxTuple) {
        *self.edge_counts.get_mut(&tuple.edge).unwrap() += 1;
        ...
        // ⚡ Bolt: Defer taking ownership to the end so we can safely use
        // references to tuple's strings during node existence checks above,
        // avoiding redundant .clone() allocations in this hot loop.
