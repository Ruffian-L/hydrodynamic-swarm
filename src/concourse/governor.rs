...
impl ActiveCell {
    ...
    pub fn add_edge(&mut self, tuple: FluxTuple) {
        *self.edge_counts.get_mut(&tuple.edge).unwrap() += 1;
        ...
        // ⚡ Bolt: Defer push to avoid cloning FluxTuple, transferring ownership directly into collections to prevent redundant string heap allocations in the hot path
