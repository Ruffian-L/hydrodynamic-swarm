...
impl PrimeGovernor {
    ...
    async fn poll_viscosity(&mut self) -> SwarmResult<()> {
        ...
        // Update cognitive state and check stability
        let is_stable = {
            // ⚡ Bolt: Compute read conditions while still holding the initial write lock guard
            // to avoid unnecessary context switching and lock contention.
