...
impl PrimeGovernor {
    ...
    async fn poll_viscosity(&mut self) -> SwarmResult<()> {
        ...
        // Update cognitive state and check Lyapunov stability in a single lock acquisition
        // ⚡ Bolt: Computed read conditions while still holding the initial write lock guard
        // to guarantee atomicity, reduce lock contention, and eliminate context switching overhead.
        let is_stable = {
    ...
    async fn execute_splat(
        ...
        let _macro_node = format!(
            ...
            node_count, edge_count, phi, delta_c
        );
