...
impl PrimeGovernor {
    ...
    async fn poll_viscosity(&mut self) -> SwarmResult<()> {
        ...
        // Update cognitive state and check Lyapunov stability atomically
        let is_stable = {
            // ⚡ Bolt: Check Lyapunov stability while holding the write guard
            // to avoid dropping it and immediately re-acquiring a read guard,
            // which reduces lock contention and context-switching overhead.
