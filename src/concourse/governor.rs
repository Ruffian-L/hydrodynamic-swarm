...
impl PrimeGovernor {
    ...
    async fn poll_viscosity(&mut self) -> SwarmResult<()> {
        ...
        // Update cognitive state and check Lyapunov stability under a single lock acquisition
        let is_stable = {
            // ⚡ Bolt: Check Lyapunov stability while still holding the write lock guard to avoid
            // a redundant context switch and re-acquisition of the read lock.
