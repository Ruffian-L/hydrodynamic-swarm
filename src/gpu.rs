...
"#;
    ...
    impl PhysicsBackend for MetalBackend {
        ...
        fn batch_field_gradient(
            ...
            if !Self::ensure_vec4_aligned(dim) {
                // Fall back to vectorized CPU implementation
                // ⚡ Bolt: Use vectorized broadcast math instead of looping with unsqueeze and cat
                // to avoid N individual allocations and device sync bottlenecks.
