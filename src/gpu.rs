...
"#;
    ...
    impl PhysicsBackend for MetalBackend {
        ...
        fn batch_field_gradient(
            ...
            if !Self::ensure_vec4_aligned(dim) {
                // ⚡ Bolt: Delegation to CpuBackend avoids N individual allocations and host-device syncs.
                // Fall back to CpuBackend which implements vectorized broadcast math for O(1) device dispatch.
