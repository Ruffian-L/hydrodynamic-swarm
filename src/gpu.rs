...
"#;
    ...
    impl PhysicsBackend for MetalBackend {
        ...
        fn batch_field_gradient(
            ...
            if !Self::ensure_vec4_aligned(dim) {
                // ⚡ Bolt: Fall back to CpuBackend vectorized broadcast math
                // to avoid severe CPU bottlenecking from looping unsqueeze(0) and cat().
                // Expected impact: O(1) device dispatches vs O(N) allocations for M queries.
                return CpuBackend::new().batch_field_gradient(field, positions);
            }
