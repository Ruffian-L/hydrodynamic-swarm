...
use crate::{gemma, gemma4, llama, qwen35};
...
impl Model {
    pub fn forward(&mut self, tokens: &Tensor, index_pos: usize) -> candle_core::Result<Tensor> {
        match self {
            ...
            Model::Qwen35(m) => m.forward(tokens, index_pos),
        }
    }
    ...
    pub fn forward_with_hidden(
        ...
        match self {
            ...
            Model::Qwen35(m) => m.forward_with_hidden(tokens, index_pos),
        }
    }
    ...
    pub fn forward_with_hidden_hooked(
        ...
        match self {
            ...
            Model::Qwen35(m) => m.forward_with_hidden_hooked(tokens, index_pos, hook),
        }
    }
    ...
    pub fn project_to_logits(&self, hidden: &Tensor) -> candle_core::Result<Tensor> {
        match self {
            ...
            Model::Qwen35(m) => m.project_to_logits(hidden),
        }
    }
    ...
    pub fn unembed(&self, hidden: &Tensor) -> candle_core::Result<Tensor> {
        match self {
            ...
            Model::Qwen35(m) => m.unembed(hidden),
        }
    }
    ...
    pub fn token_embeddings(&self) -> &Tensor {
        match self {
            ...
            Model::Qwen35(m) => m.token_embeddings(),
        }
    }
    ...
    pub fn clear_kv_cache(&mut self) {
        match self {
            ...
            Model::Qwen35(m) => m.clear_kv_cache(),
        }
    }
}
...
pub fn load_gguf(
    ...
    let load_qwen35 = arch.contains("qwen35");
    let load_gemma4 =
        !load_qwen35 && (arch.contains("gemma4") || (arch.is_empty() && path_looks_like_gemma4(model_path)));
    ...
    let model = if load_qwen35 {
        if verbose {
            println!("    Architecture: {} → Qwen35 hybrid loader", describe());
        }
        Model::Qwen35(qwen35::ModelWeights::from_gguf(ct, &mut reader, device)?)
    } else if load_gemma4 {
        if verbose {
            println!(
    ...
    let tokenizer_fallbacks: &[&str] = if load_qwen35 {
        &["qwen35.tokenizer.json", "qwen.tokenizer.json", "data/qwen.tokenizer.json"]
    } else if load_gemma4 {
