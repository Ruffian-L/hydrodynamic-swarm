...
impl JacobianLens {
    ...
    pub fn measure(
        ...
        if let Some(trace) = self.trace.as_mut() {
            let summary = format!(
                ...
                report.global_sensitivity, report.dominant_dimensions, report.dominant_tokens
            );
    ...
    fn measure_site(
        ...
        let mut sensitivity_matrix: Vec<f32> =
            vec![0.0; top_k_tokens.len() * dims_to_measure.len()];
        ...
        for (dim_idx, &dim) in dims_to_measure.iter().enumerate() {
            ...
            let delta = Tensor::from_vec(onehot, (1, hidden_dim), dev)?.to_dtype(hidden.dtype())?;
    ...
    fn compute_report(
        ...
        let global_sensitivity: f32 =
            measurements.iter().map(|m| m.norm).sum::<f32>() / measurements.len() as f32;

        // Dominant dimensions: aggregate across all sites
        // (simplified: take union of top dimensions from all sites)
        let mut dim_sensitivity_sum: std::collections::HashMap<usize, f32> =
            std::collections::HashMap::new();
        let mut dim_sensitivity_count: std::collections::HashMap<usize, f32> =
            std::collections::HashMap::new();
        ...
        let mut dominant_dimensions: Vec<(usize, f32)> = dim_sensitivity_sum
            .iter()
            .map(|(dim, sum)| {
        ...
        let mut token_sensitivity_sum: std::collections::HashMap<usize, f32> =
            std::collections::HashMap::new();
        let mut token_sensitivity_count: std::collections::HashMap<usize, f32> =
            std::collections::HashMap::new();
        ...
        let mut dominant_tokens: Vec<(usize, f32)> = token_sensitivity_sum
            .iter()
            .map(|(token_id, sum)| {
...
impl DimSignature {
    ...
    pub fn l2_norm(&self) -> f32 {
        self.dims.iter().map(|(_, w)| w * w).sum::<f32>().sqrt()
    }
}
...
impl JacobianKey {
    pub fn new(signature: DimSignature, phase: KeyPhase, step: usize, residual_d: usize) -> Self {
        Self {
            signature,
            phase,
            step,
...
pub fn sparse_cosine(a: &DimSignature, b: &DimSignature) -> f32 {
    ...
    let map_b: std::collections::HashMap<usize, f32> = b.dims.iter().copied().collect();
    let mut dot = 0.0f32;
    for &(d, wa) in &a.dims {
...
impl MultiKeyAddress {
    ...
    pub fn emit_pick_query(&self, top_k: usize) -> PickQuery {
        ...
        clusters.sort_by(|a, b| {
            b.key_indices.len().cmp(&a.key_indices.len()).then_with(|| {
                let sa = a
                    .key_indices
                    .first()
                    .map(|&i| self.keys[i].step)
                    .unwrap_or(0);
                let sb = b
                    .key_indices
                    .first()
...
mod multi_key_tests {
    ...
    fn multi_key_emit_pick_query_nonempty() {
        ...
        addr.push(
            JacobianKey::new(sig(&[(200, 1.0), (201, 0.5)]), KeyPhase::Settle, 48, 3840)
                .with_turn(0),
        );
