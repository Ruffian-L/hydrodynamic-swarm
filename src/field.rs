...
impl ContinuousField {
    ...
    pub fn probe_gradient_topk(&self, pos: &Tensor, k: usize) -> Result<Tensor> {
        ...
        // ⚡ Bolt: Use index_select instead of get() + cat() for O(1) host-device sync
        // and to avoid O(K) intermediate tensor allocations
        let topk_indices: Vec<u32> = indexed[..k].iter().map(|&(i, _)| i as u32).collect();
        let index_tensor = Tensor::from_vec(topk_indices, k, &self.device)?;
        let topk_positions = self.positions.index_select(&index_tensor, 0)?;
