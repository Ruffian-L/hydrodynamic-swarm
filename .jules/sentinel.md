...
## 2026-05-25 - Unbounded Map Growth and Panic in State Arrays
**Vulnerability:** Using `.unwrap()` on container lookups in high-frequency event handlers poses a DoS crash risk if unknown keys are passed. Replacing it with `.entry().or_insert()` prevents panics but introduces an Out-Of-Memory (OOM) DoS vulnerability by allowing unbounded map growth.
**Learning:** In highly concurrent event-driven architectures, state maps should not implicitly grow from unvalidated external inputs.
**Prevention:** Prefer pattern matching (e.g., `if let Some()`) for container lookups to ignore unknown keys safely, preventing both application panics and memory exhaustion attacks.
