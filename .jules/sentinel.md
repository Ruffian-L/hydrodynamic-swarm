...
## 2024-05-24 - Unbounded Panic in Map Lookup
**Vulnerability:** The `add_edge` function in `src/concourse/governor.rs` uses `.unwrap()` when looking up an edge in `edge_counts`. If an unknown edge type is passed to `add_edge`, the application will panic and crash, causing a Denial of Service (DoS).
**Learning:** Using `.unwrap()` on map lookups or arrays is extremely dangerous in highly concurrent event-driven architectures. It allows malicious or malformed input to easily crash the application.
**Prevention:** Prefer pattern matching with `if let Some()` to handle missing keys gracefully instead of panicking, or safely insert default values without allowing unbounded map growth.
