...
## 2024-05-24 - Missing Timeout on External LLM API
**Vulnerability:** The `GrokOracle` network client (`reqwest::Client::new()`) lacked an explicit timeout, creating a resource exhaustion (DoS) vulnerability. If the external xAI/Grok API became unresponsive, the application would hang indefinitely.
**Learning:** Default HTTP clients often do not enforce strict timeouts. In a concurrent or event-driven system, blocking indefinitely on external services can lock up worker threads and halt the swarm.
**Prevention:** Always explicitly configure timeouts for external network requests using `Client::builder().timeout(std::time::Duration::from_secs(60)).build()?`.
