...
## 2026-08-18 - Lock Poisoning DoS Risk in CacheManager
**Vulnerability:** The `CacheManager` in `src/concourse/cache.rs` accesses internal `RwLock` instances (for both LRU and TTL caches) using `.read().unwrap()` and `.write().unwrap()`. If a thread panics while holding either of these locks, the lock becomes poisoned. Subsequent threads calling `.unwrap()` on lock acquisition will also panic, causing a cascading failure that leads to a Denial of Service (DoS) for the entire application.
**Learning:** Using standard `.unwrap()` on lock acquisitions (like `Mutex` or `RwLock`) creates fragile concurrency boundaries. A single panicked thread can render shared resources permanently inaccessible and crash other dependent threads.
**Prevention:** Always use `.unwrap_or_else(|e| e.into_inner())` when acquiring locks (yielding `LockResult`) to safely recover the lock guard and ignore the poison state, ensuring the system remains resilient against isolated thread panics.
