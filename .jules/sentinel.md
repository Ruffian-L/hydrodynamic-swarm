...
## 2025-02-27 - [Lock Poisoning DoS Vulnerability]
**Vulnerability:** Found multiple instances of `.unwrap()` calls on `RwLock::read()`, `RwLock::write()`, and `Mutex::lock()` operations in asynchronous and multithreaded contexts (like cache management and LLM model wrappers).
**Learning:** If a thread panics while holding the lock, it becomes poisoned. The `.unwrap()` on subsequent lock acquisitions will cause those threads to panic as well, propagating the failure and creating a potential Denial of Service (DoS) vulnerability that takes down the entire application or service layer.
**Prevention:** Always use `.unwrap_or_else(|e| e.into_inner())` on thread-synchronization locks to safely recover the lock guard and ignore the poison state, ensuring application resilience against isolated panics.
