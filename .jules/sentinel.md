...
## 2026-08-15 - Safe Lock Acquisition to Prevent Poisoning
**Vulnerability:** Several concurrent components (`CacheManager`, `GemmaModel`, `InstructGemmaModel`) were calling `.unwrap()` directly on `Mutex` and `RwLock` acquisitions (`lock()`, `read()`, `write()`). If any thread panicked while holding the lock, it would poison the lock, causing all subsequent `unwrap()` calls on that lock across the entire application to panic, leading to a permanent Denial of Service (DoS) vulnerability.
**Learning:** In Rust, `.unwrap()` on a `LockResult` from `std::sync::Mutex` or `std::sync::RwLock` will panic if the lock is poisoned (i.e., a thread panicked while holding it). This is a common pattern that easily introduces cascading failures in highly concurrent applications like this one.
**Prevention:** Always handle lock poisoning explicitly. To safely recover and continue operating even if a previous thread panicked, use `.unwrap_or_else(|e| e.into_inner())` on the `LockResult`. This ignores the poison state and grants access to the underlying data, preventing the cascading DoS.
