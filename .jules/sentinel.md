...
## 2024-05-24 - DoS vulnerability via map lookup unwrap
**Vulnerability:** `ActiveCell::add_edge` uses `*self.edge_counts.get_mut(&tuple.edge).unwrap() += 1;` which panics and crashes the application if a dynamically generated edge is not pre-populated in the `edge_counts` map. This is a severe DoS vulnerability in event-driven systems where unknown keys can trigger application termination.
**Learning:** In highly concurrent architectures, never use `.unwrap()` on container lookups.
**Prevention:** Use pattern matching like `if let Some(count) = self.edge_counts.get_mut(...)` instead of `unwrap()`, or avoid `.entry().or_insert()` to prevent Out-Of-Memory (OOM) DoS vulnerabilities if an attacker spams unknown keys.
