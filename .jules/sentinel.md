...
## 2024-05-24 - Unbounded Map Growth vs Panic in Event-Driven Architecture
**Vulnerability:** Using `unwrap()` on hash map lookups for external or rapidly-ingested event data (like `tuple.edge` in `add_edge`) creates a trivial DoS vector via application panic. Conversely, using `.entry().or_insert()` to fix the panic introduces an Out-Of-Memory (OOM) DoS vulnerability if an attacker spams unknown keys.
**Learning:** In high-concurrency systems, neither panicking nor unbounded state growth is acceptable.
**Prevention:** Always use pattern matching (`if let Some()`) to safely ignore unknown keys, preventing both crashes and unbounded memory exhaustion.
