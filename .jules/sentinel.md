...
## 2024-04-19 - Missing timeout configurations
**Vulnerability:** The reqwest::Client in src/grok_oracle.rs was instantiated without a timeout configuration.
**Learning:** External API dependencies can hang indefinitely and cause resource exhaustion if timeouts are not explicitly configured.
**Prevention:** Always use .timeout(std::time::Duration::from_secs(X)) when configuring network clients.
