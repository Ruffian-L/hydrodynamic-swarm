...
## 2024-05-24 - Lock Poisoning DoS
**Vulnerability:** Calling `.unwrap()` on `Mutex::lock()`, `RwLock::read()`, or `RwLock::write()` causes a panic if the lock is poisoned (i.e., another thread panicked while holding it). This can be exploited by an attacker to crash the entire application (Denial of Service) by intentionally triggering a panic in one thread that poisons a shared lock.
**Learning:** In Rust, lock poisoning is a safety mechanism, but crashing the application on access is often undesirable for long-running services. Panicking on lock acquisition turns a single-thread failure into a full system DoS.
**Prevention:** Instead of `.unwrap()`, use `.unwrap_or_else(|e| e.into_inner())` to safely recover the lock guard and ignore the poison state, allowing the application to continue functioning even if a previous thread panicked.
