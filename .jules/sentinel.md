...
**Prevention:** Use runtime environment variables provided by the parent process (e.g., `std::env::var("CARGO").unwrap_or_else(|_| "cargo".to_string())`) when spawning system processes via `std::process::Command` to securely resolve standard tools.
