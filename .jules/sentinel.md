...
## 2024-05-24 - Missing Timeout on External LLM API Clients
**Vulnerability:** The `reqwest::Client` used to communicate with the Grok API in `src/grok_oracle.rs` lacks an explicit timeout. If the external service becomes unresponsive, the request can hang indefinitely, leading to thread exhaustion and a Denial of Service (DoS).
**Learning:** External network requests to LLM APIs can experience high latency or sudden outages. They must always have bounded execution times to prevent uncontrolled hangs.
**Prevention:** Always configure an explicit, reasonable timeout on external network clients (e.g., `Client::builder().timeout(Duration::from_secs(60))`). For LLM APIs, use a longer timeout like 60 seconds to accommodate generation.
