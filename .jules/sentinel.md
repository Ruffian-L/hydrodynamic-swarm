...
## 2025-02-27 - Fix lock poisoning vulnerability in cache manager
**Vulnerability:** DoS (Denial of Service) via lock poisoning in `CacheManager`. `RwLock` operations were using `.unwrap()`, which would cause a panic and bring down the application if a lock became poisoned (which happens if a thread panics while holding the lock).
**Learning:** In Rust, `RwLock::write().unwrap()` or `RwLock::read().unwrap()` is unsafe for long-running services. If any thread panics while holding a lock, the lock gets poisoned, and subsequent `.unwrap()` calls will cause a cascade of panics in other threads trying to acquire the lock.
**Prevention:** Use `.unwrap_or_else(|e| e.into_inner())` on lock acquisitions to safely recover the lock guard and ignore the poison state, ensuring the service remains resilient even if a thread previously panicked.
