...
## 2024-05-24 - Lock Poisoning via unwrap()
**Vulnerability:** Calling `.unwrap()` on `RwLock` or `Mutex` lock acquisitions causes the thread to panic if the lock is poisoned (i.e., another thread panicked while holding it). This creates a Denial of Service (DoS) vulnerability where a single thread failure cascades into a complete application crash.
**Learning:** In Rust, an intentional panic inside a synchronization primitive closure or dropping a poisoned lock terminates the application if `.unwrap()` is used on subsequent lock acquisitions. This is a critical stability and security flaw in concurrent environments.
**Prevention:** Use `.unwrap_or_else(|e| e.into_inner())` when acquiring locks to safely recover the lock guard and ignore the poison state, ensuring the application remains resilient to individual thread panics.
