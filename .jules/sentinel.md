...
## 2024-05-24 - Unbounded HTTP Client Timeout
**Vulnerability:** The `reqwest::Client` in `src/grok_oracle.rs` was initialized using `Client::new()` without an explicit timeout.
**Learning:** Default HTTP clients do not have timeouts, allowing external APIs (like xAI/Grok) to cause infinite hangs and resource exhaustion Denial of Service (DoS) if they become unresponsive.
**Prevention:** Always configure network clients with an explicit timeout using `.timeout(std::time::Duration::from_secs(X))`. For LLM API clients, use a longer duration like 60 seconds to accommodate extensive text completions.
