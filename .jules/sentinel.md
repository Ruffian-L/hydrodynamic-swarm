...
## 2025-02-27 - [Prevent DoS via Lock Poisoning in Cache]
**Vulnerability:** Found multiple `unwrap()` calls on `RwLock` operations (`write().unwrap()`, `read().unwrap()`) within `src/concourse/cache.rs`. A panic in a thread while holding the lock would poison the lock, causing subsequent threads that attempt to acquire the lock using `unwrap()` to also panic. This cascades into a Denial of Service (DoS) across the application.
**Learning:** In highly concurrent environments (like async servers), standard library concurrency primitives (like `RwLock` or `Mutex`) become vulnerable to poisoning.
**Prevention:** Avoid calling `.unwrap()` on lock acquisitions. Instead, safely recover the lock guard by handling the `PoisonError` state with `.unwrap_or_else(|e| e.into_inner())` to ignore the poison and gracefully continue operations, maintaining uptime even if an internal invariant temporarily caused a panic elsewhere.
