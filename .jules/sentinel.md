...
## 2024-05-25 - Unbounded Map Growth and Panic DoS in Swarm Governor
**Vulnerability:** Using `.unwrap()` on container lookups in high-throughput event handlers (like `governor.rs` processing `FluxTuple` edges) causes application panics when unknown keys are received, creating a direct Denial of Service (DoS) vulnerability.
**Learning:** Preventing panics by using `.entry().or_insert()` or similar unbounded map insertions introduces an Out-Of-Memory (OOM) DoS vulnerability by allowing an attacker to spam unknown keys and exhaust memory.
**Prevention:** Use pattern matching (`if let Some()`) for container lookups in event handlers. This prevents both application crashes from missing keys and unbounded memory growth from malicious payloads.
