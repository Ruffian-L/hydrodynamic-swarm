...
## 2026-08-05 - Prevent DoS from Poisoned OnceLock via Panic
**Vulnerability:** Found an intentional `panic!` inside a `std::sync::OnceLock::get_or_init` closure (used for initializing `InstructGemmaModel`) as a flawed attempt to avoid caching a failed initialization.
**Learning:** In Rust, an intentional panic inside a `std::sync::OnceLock::get_or_init` closure cannot be caught or recovered from; it will poison the lock and terminate the application. This introduces a Denial of Service (DoS) vulnerability.
**Prevention:** To prevent DoS vulnerabilities, avoid panicking in static initialization contexts and instead use patterns that allow for graceful error handling or fallback, such as wrapping the initialization logic in a separate function that safely updates the `OnceLock` on success without panicking on failure.
