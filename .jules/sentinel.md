...
## 2024-05-24 - Unbounded Map Lookup Panic in ActiveCell::add_edge
**Vulnerability:** In `src/concourse/governor.rs`, `ActiveCell::add_edge` uses `.get_mut(&tuple.edge).unwrap()` to increment edge counts. If a `FluxTuple` contains an uninitialized or unknown `RelationalEdge`, it will panic and crash the active cell governor (Denial of Service).
**Learning:** Using `unwrap()` on dictionary/map lookups in highly concurrent event-driven architectures like Hydrodynamic Swarm is a major DoS vulnerability.
**Prevention:** Use pattern matching like `if let Some(count) = self.edge_counts.get_mut(&tuple.edge) { ... }` to gracefully handle unknown states instead of panicking or unconditionally inserting keys that could cause OOM.
