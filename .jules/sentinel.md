...
## 2023-10-27 - [Poisoned Locks leading to DoS]
**Vulnerability:** DoS vulnerability due to lock poisoning. Calling `.unwrap()` on `RwLock` or `Mutex` methods like `.read()` or `.write()` can cause the application to panic and crash if another thread panicked while holding the lock.
**Learning:** Panics in Rust propagate across thread boundaries when using shared state via `Mutex` or `RwLock`. If a thread panics while holding a lock, the lock becomes "poisoned", returning an `Err` on subsequent acquisition attempts. Using `.unwrap()` on the lock acquisition turns this expected failure mode into an uncontrolled panic, crashing the entire application instead of gracefully recovering or handling the error.
**Prevention:** Instead of using `.unwrap()` on lock acquisitions, use `.unwrap_or_else(|e| e.into_inner())` to safely recover the lock guard, ignoring the poison state, or properly handle the lock error.
