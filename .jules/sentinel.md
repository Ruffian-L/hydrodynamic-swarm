...
**Vulnerability:** In highly concurrent event-driven architectures like the Hydrodynamic Swarm (e.g., `src/concourse/governor.rs`), using `unwrap()` on container lookups or state arrays poses a major Denial of Service (DoS) vulnerability via application crashes/panics if unknown keys are passed.
**Learning:** Attempting to solve this with `.entry().or_insert()` prevents panics, but can introduce an Out-Of-Memory (OOM) DoS vulnerability by allowing unbounded map growth if an attacker spams unknown keys.
**Prevention:** Prefer pattern matching (e.g., `if let Some()`) over `.entry().or_insert()` or `.unwrap()` to safely handle missing keys without unbounded allocations.
