...
## 2024-05-24 - Denial of Service via Unwrap Panics
...
**Learning:** Never use `unwrap()` when accessing state that could be dynamically modified.
**Prevention:** Safer alternatives like `.entry().or_insert()` or pattern matching (e.g., `if let`) should be used when accessing state that could be dynamically modified.
