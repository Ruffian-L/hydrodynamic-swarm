...
## 2026-05-15 - DoS via Map Lookup Panic
**Vulnerability:** The `ActiveCell::add_edge` function invoked `.unwrap()` on a HashMap mutable lookup `get_mut()`. This created a critical Denial of Service (DoS) vulnerability where any unhandled edge types (or a maliciously injected edge) would panic the Prime Governor daemon.
**Learning:** In highly concurrent architectures, using `.unwrap()` on container lookups creates crash vectors.
**Prevention:** Always use safe access patterns like `if let Some()` instead of `.unwrap()`. Avoid `.entry().or_insert()` to mitigate Out-Of-Memory map growth vulnerabilities if unknown keys can be supplied.
