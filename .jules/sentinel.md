...
## 2024-05-18 - Prevent DoS via Lock Poisoning in CacheManager
**Vulnerability:** The CacheManager implementation in `src/concourse/cache.rs` uses `.write().unwrap()` and `.read().unwrap()` to acquire RwLock guards on caches. If a panic occurs while a lock is held by another thread, the lock becomes poisoned. Subsequent calls to `.unwrap()` will panic, causing a cascade of thread failures and ultimately a Denial of Service (DoS) of the entire application.
**Learning:** Using `.unwrap()` on `RwLock` or `Mutex` lock acquisition results is an anti-pattern in Rust that turns recoverable thread panics into application-wide crashes due to lock poisoning.
**Prevention:** Always handle lock poisoning gracefully. Use `.unwrap_or_else(|e| e.into_inner())` to recover the lock guard when appropriate, or return the error up the call stack if state invariants might be compromised.
