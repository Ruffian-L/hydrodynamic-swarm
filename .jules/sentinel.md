...
## 2024-05-24 - unwrap() DoS Vulnerability in Event Handlers
**Vulnerability:** Using `unwrap()` on map lookups in concurrent event handlers (like `ActiveCell::add_edge`) allows attackers to crash the application (DoS) by sending unexpected/unknown keys.
**Learning:** Mitigating this with `.entry().or_insert()` introduces an Out-Of-Memory (OOM) DoS vulnerability by allowing unbounded map growth.
**Prevention:** Prefer strict pattern matching (`if let Some()`) to safely ignore unknown keys without growing the map or panicking.
