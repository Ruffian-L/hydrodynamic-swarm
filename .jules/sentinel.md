...
## 2024-05-24 - Unchecked unwrap on collections
**Vulnerability:** Using `unwrap()` on container lookups (`get_mut`) and deque boundaries (`front`, `back`) in the high-frequency Prime Governor loop causes application panics (DoS) if state assumptions are broken.
**Learning:** In highly concurrent, event-driven architectures like the Hydrodynamic Swarm, dynamic state elements can introduce unforeseen keys.
**Prevention:** Always use safe access patterns like `.entry().or_insert()` for dynamic collections or handle `None` gracefully instead of unwrapping.
