...
## 2024-05-24 - Missing Timeout Resource Exhaustion
**Vulnerability:** The `reqwest::Client` in `GrokOracle` was created without a timeout, meaning it could hang indefinitely if the external Grok API becomes unresponsive.
**Learning:** Network calls without strict timeouts can lead to task exhaustion, thread blocking, and a Denial of Service (DoS) of the application.
**Prevention:** Always explicitly set a reasonable timeout using `.timeout(std::time::Duration::from_secs(X))` when configuring network clients.
