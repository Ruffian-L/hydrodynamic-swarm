...
## 2024-05-25 - DoS via Map Lookup Unwrap
**Vulnerability:** Using `unwrap()` on container lookups like `edge_counts.get_mut(&key).unwrap()` causes application panics (Denial of Service) if an unexpected or unknown key is provided.
**Learning:** In highly concurrent architectures, panicking on invalid state from external/event-driven inputs is a major availability risk.
**Prevention:** Always use safe pattern matching (e.g., `if let Some()`). Avoid `.entry().or_insert()` when mitigating this, as unbounded map growth introduces an Out-Of-Memory (OOM) DoS vulnerability.
