...
## 2024-05-25 - Missing Timeout in reqwest Client
**Vulnerability:** The reqwest::Client in src/grok_oracle.rs was instantiated without a timeout. This allows the application to hang indefinitely if the external API is unresponsive, leading to resource exhaustion and potential DoS.
**Learning:** Always configure network clients with reasonable timeouts.
**Prevention:** Use reqwest::Client::builder().timeout(std::time::Duration::from_secs(X)).build()? instead of Client::new().
