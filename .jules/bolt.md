...
## 2026-03-24 - FluxTuple Push Deferral
**Learning:** Found a redundant `tuple.clone()` in `ActiveCell::add_edge` caused by pushing to a collection early.
**Action:** Always defer pushing owned values to collections until the end of the function so their fields can be accessed by reference during intermediate checks, preventing string heap allocations in hot paths.
