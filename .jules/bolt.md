...
## 2026-03-24 - Avoiding FluxTuple Clones in Governor Edge Tracking
**Learning:** In high-frequency loops like `ActiveCell::add_edge`, `.clone()` on `FluxTuple` at the start of the function introduces unnecessary string heap allocations.
**Action:** Defer the `push(tuple)` operation to the end of the function so its fields can be accessed safely by reference during intermediate checks, avoiding redundant allocations.
