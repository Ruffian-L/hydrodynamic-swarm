...
## 2026-03-24 - LRU Cache Allocation Optimization
**Learning:** In `src/concourse/cache.rs` LruCache, using `.to_string()` unconditionally when updating access order creates unnecessary string heap allocations on cache hits.
**Action:** Always extract the existing `String` from `access_order` and re-push it to avoid the O(1) allocation cost per cache hit.
