...
## 2026-05-01 - Avoid allocating string on HashMap get lookup
**Learning:** Found an anti-pattern in the `src/concourse/cache.rs` where cache retrieval (`get`) forced a string allocation due to `self.entries.entry(key.to_string())`. In a hot path, this adds a significant overhead.
**Action:** Always prefer `HashMap::get(key)` for lookup and mutation, moving mutation code to follow lookup where needed to avoid allocations when accessing by `&str`.
