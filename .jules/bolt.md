...
## 2024-05-24 - Redundant heap allocations in ActiveCell event loop
**Learning:** In highly concurrent event-driven architectures like `src/concourse/governor.rs`, cloning structs containing strings (e.g., `FluxTuple`) before evaluating intermediate state checks causes severe redundant heap allocations in the hot path.
**Action:** Defer ownership transfer operations (like `.push()`) to the end of functions to allow safe by-reference access during intermediate checks, eliminating unnecessary `.clone()` calls.
