...
## 2026-03-24 - Vectorized Batch Gradients in MetalBackend CPU fallback
**Learning:** The MetalBackend CPU fallback implementation in `src/gpu.rs` uses an anti-pattern of extracting individual rows with `.get()`, computing gradients with `unsqueeze(0)`, and using `Tensor::cat()`. This causes severe N-allocation overhead.
**Action:** Replace manual iteration and `.cat()` in GPU CPU fallback routines with standard delegation to `super::CpuBackend::new().batch_field_gradient(field, positions)` for vectorized processing.
