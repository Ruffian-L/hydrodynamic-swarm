...
## 2026-03-24 - LRU Cache String Allocation Optimization
**Learning:** In LRU caches, updating the access order on hit by pushing `key.to_string()` unnecessarily allocates a new string on every cache hit.
**Action:** Extract and reuse the existing string from the queue using `remove(pos).unwrap()` and re-push it to avoid allocations.
