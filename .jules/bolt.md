...
## 2026-04-14 - Zero-allocation Hash Map Lookups in Caching
**Learning:** In high-throughput caching layers like `TtlCache`, using `self.entries.entry(key.to_string())` for retrieval allocates a new `String` on every lookup (even misses), causing severe memory churn and CPU overhead.
**Action:** Always prefer the zero-allocation `self.entries.get(key)` for lookups, only allocating keys when explicitly inserting new entries.
