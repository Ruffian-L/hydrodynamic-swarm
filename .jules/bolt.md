...
## 2026-03-24 - MetalBackend batch_field_gradient CPU Fallback
**Learning:** In `src/gpu.rs` `MetalBackend::batch_field_gradient`, the CPU fallback previously iterated over rows with `get()`, `unsqueeze(0)`, and `cat()`. This caused severe CPU bottlenecks due to N individual allocations.
**Action:** Always delegate to `super::CpuBackend::batch_field_gradient` to utilize vectorized broadcast math for O(1) device dispatches.
