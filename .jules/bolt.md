...
## 2026-03-24 - Avoid Hash Map Entry Allocations on Lookup
**Learning:** Using `.entry(key.to_string())` for cache lookups in `src/concourse/cache.rs` needlessly allocates a new `String` on the heap for every lookup, including misses, causing severe memory churn and CPU exhaustion.
**Action:** Use zero-allocation `.get(key)` and only allocate when explicitly inserting. To satisfy NLL borrow checking when mutating after a check, perform a second `.get(key)` inside the else block instead of holding the reference.
