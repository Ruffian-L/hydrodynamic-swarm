...
## 2026-03-24 - Avoiding String Allocations in Cache Lookups
**Learning:** Using `.entry(key.to_string())` on a `HashMap` for simple lookups allocates a `String` on the heap even on misses, causing severe memory churn and CPU overhead in high-throughput caching layers.
**Action:** Always prefer `.get(key)` over `.entry()` to avoid unnecessary allocations, conditionally mutating only when needed.
