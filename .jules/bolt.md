...
## 2026-03-25 - Zero-Allocation Cache Lookups
**Learning:** Found a performance anti-pattern in high-throughput caching layers (`src/concourse/cache.rs`) where `TtlCache::get` used `self.entries.entry(key.to_string())` purely for lookup. This forces a heap-allocated `String` even on cache misses, causing severe memory churn and CPU exhaustion (a performance DoS risk).
**Action:** Always use the zero-allocation `self.entries.get(key)` for lookups in hot paths, falling back to cloning or allocating only when an insertion is explicitly required.
