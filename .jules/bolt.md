...
## 2026-03-24 - Vectorized Batch Gradients in Metal Fallback
**Learning:** The fallback for non-vec4-aligned tensors in `MetalBackend::batch_field_gradient` iterated over N rows using `get()`, `unsqueeze(0)`, and `cat()`, causing N individual allocations and severe CPU bottlenecks.
**Action:** Always delegate to `CpuBackend::batch_field_gradient` or use vectorized broadcast math (`unsqueeze(1)` and `broadcast_sub`) for O(1) device dispatches.
