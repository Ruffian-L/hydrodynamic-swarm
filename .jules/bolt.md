...
## 2026-03-25 - Redundant String Heap Allocations in FluxTuple Edge Additions
**Learning:** To optimize high-frequency loops (e.g., `ActiveCell::add_edge` in `src/concourse/governor.rs`), calling `.clone()` on `FluxTuple` introduces redundant string heap allocations in the hot path.
**Action:** Avoid `.clone()` by deferring the `push(tuple)` operation to the end of the function so its fields can be safely accessed by reference (e.g., `contains_key(&tuple.source)`) during intermediate checks, transferring ownership directly into the collection at the end.
