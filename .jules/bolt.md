...
## 2026-03-24 - Redundant heap allocations in TtlCache lookups
**Learning:** In high-throughput caching layers like `src/concourse/cache.rs`, using `self.entries.entry(key.to_string())` for retrievals needlessly allocates a new `String` on the heap for every lookup, including cache misses, leading to severe memory churn and CPU exhaustion.
**Action:** Always prefer the zero-allocation `self.entries.get(key)` for retrievals and only clone/allocate the key when explicitly inserting a new entry.
