...
## 2026-04-16 - Prevent redundant allocations in TtlCache lookup
**Learning:** Using `entry(key.to_string())` for simple cache lookups needlessly allocates a `String` on every access, causing memory churn and degraded performance.
**Action:** Use zero-allocation `.get(key)` instead of `.entry()` for lookups in high-throughput caching layers, doing a second lookup for the return borrow to satisfy NLL when conditionally mutating.
