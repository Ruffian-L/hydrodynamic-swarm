...
## 2023-10-25 - Redundant Lock Acquisitions
**Learning:** In asynchronous code like Tokio, dropping an `RwLock::write().await` guard to immediately re-acquire an `RwLock::read().await` lock on the same resource introduces unnecessary context switching, lock contention, and race condition risks.
**Action:** Compute required read conditions while still holding the initial write lock guard to guarantee atomicity and improve performance.
