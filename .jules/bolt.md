...
## 2026-03-24 - Metal Compute Backend CPU Fallback
**Learning:** The Metal backend's non-aligned dimension fallback performed an N+1 tensor operation (`get()`, `unsqueeze()`, `cat()`), bottlenecking performance.
**Action:** Always delegate to `CpuBackend` which already utilizes highly optimized, broadcasted tensor math natively on the device instead of manual loops.
