...
## 2026-03-24 - Zero-Allocation Cache Lookups
**Learning:** Using `HashMap::entry(key.to_string())` for cache lookups causes severe memory churn by needlessly allocating a new `String` on every hit and miss. Furthermore, `VecDeque` queues like LRU access orders should not call `key.to_string()` on existing elements.
**Action:** Replace `entry()` with `.get(key)` for cache reads. When conditionally mutating an entry, extract the state first (e.g., `let expired = entry.is_expired()`), perform the mutation if needed, and call `.get()` again to return the reference, satisfying NLL without allocations. For LRU string queues, extract and re-insert the existing `String` using `remove(pos)` and `push_back(existing)`.
