...
## 2025-04-06 - Use aria-pressed on toggle buttons and group related settings
**Learning:** Custom UI buttons acting as stateful toggles are inaccessible to screen readers without `aria-pressed`. Related controls also lack `role="group"` context.
**Action:** Always add `aria-pressed="true/false"` to buttons acting as toggles, and wrap logically grouped controls in a container with `role="group"` and `aria-labelledby` or `aria-label`.
