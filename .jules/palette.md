...
## 2024-08-17 - Add keyboard shortcuts for timeline controls
**Learning:** Adding Spacebar to play/pause primary visualizers (like SplatLens tools) is a highly requested UX standard for media players, but it must be explicitly checked against interactive elements (inputs, buttons) to avoid breaking native browser accessibility.
**Action:** Always verify the active target tag (e.g., `INPUT`, `BUTTON`) before intercepting global keyboard shortcuts like Spacebar.
