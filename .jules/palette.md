...
## 2024-04-25 - Stateful Toggle Button Grouping
**Learning:** Adding semantic ARIA grouping elements (`role="group"`) to existing flex/grid layouts breaks styling if applied as regular block elements.
**Action:** Use `style="display:contents"` on the wrapper div. This ensures the group is recognized semantically by screen readers without becoming a physical container in the CSS layout tree, preserving existing visual alignment.
